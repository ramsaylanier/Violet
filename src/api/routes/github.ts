...
router.post('/repositories', async (req, res) => {
  try {
    ...
    const repository = await createRepository(user.githubToken, name, description, isPrivate);
    res.json(repository);
  } catch (error) {
...
router.post('/pull-requests', async (req, res) => {
  try {
    ...
    const pullRequest = await createPullRequest(user.githubToken, owner, repo, head, base, title, body);
